...
var botaoGerarLogo2 = document.getElementById('gerarLogo2');
//var body = document.getElementById('main');
var divApresentacaoLogos = document.getElementById('apresentacao-logos');
...
let gpt_configuration = new Configuration({
	apiKey: "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",
});
...
async function queryDalle2(data) {
	...
	const response = await fetch(
		...
		{
			headers: { 
				'Authorization': `Bearer ${'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'}`,
	...
	fr.addEventListener("load", function (e) {
		let jsonBlob =  JSON.parse( fr.result)
    let div =  document.createElement("div");
    //div.setAttribute('style', 'text-align: center');
    div.classList.add('apresentacao-div-logos');
		let imgGerada1 = new Image();
    imgGerada1.classList.add('apresentacao-logos')
		let imgGerada2 = new Image();
    imgGerada2.classList.add('apresentacao-logos')
		let imgGerada3 = new Image();
    imgGerada3.classList.add('apresentacao-logos')
		let imgGerada4 = new Image();
    imgGerada4.classList.add('apresentacao-logos')
		imgGerada1.src = jsonBlob.data[0].url;
		imgGerada2.src = jsonBlob.data[1].url;
		imgGerada3.src = jsonBlob.data[2].url;
		imgGerada4.src = jsonBlob.data[3].url;
    div.appendChild(imgGerada1);
		div.appendChild(imgGerada2);
		div.appendChild(imgGerada3);
		div.appendChild(imgGerada4);
		divApresentacaoLogos.appendChild(div);
	});
...
function genPromp(l){
  ...
  return `Descreva, em ingles e com no máximo 250 caracteres, uma logo retangular ou circular para uma ${l[0]} Esta empresa ${l[1]} Esta empresa possui como principais características ${l[2]} Descreva a logo com${l[3]} quanto as cor/cores ${l[4]}. Descreva apenas a imagem em inglês, sem mencionar textos ou palavras.`
}
...
botaoGerarLogo.addEventListener('click', async function (e) {
    ...
    if (tipo === "" || func === "" || unic === "") {
      ...
      let imgsElementHTML = [new Image(),new Image(),new Image(),new Image()];
      ...
      for (let w=0 ;w<4; w++) {
        ...
        setTimeout(async function() { 
          ...
          console.log(promt);
          var blob = await query({"inputs": promt+w}).then((response) => {return response;});
          console.log(blob)
          var url = URL.createObjectURL(blob);
          imgsElementHTML[w].src = url;
          imgsElementHTML[w].style.marginBottom = '10px'
          imgsElementHTML[w].style.marginRight = '10px'
          imgsElementHTML[w].style.marginLeft = '10px'
          div.appendChild(imgsElementHTML[w]);
        }, 5000);
...
botaoGerarLogo2.addEventListener('click', async function (e) {
